...
def main():
    filename = "../../Python-WORLD/test/test-walk.wav"#正常語速_往前走.wav"
    result = "./cut_speech/"
    ...
    print(f"nchannels: {nchannels}, {sampwidth}, {framerate}, {nframes}")
...
def cut_speech(path, num_words=0):
    ...
    while i < len(EHF2) :
        with wave.open(result + str(j) + ".wav", 'wb') as wavfile:
            wavfile.setparams((nchannels, sampwidth, framerate, 0, 'NONE', 'NONE'))
            print(round(EHF2[i] * 44100), round(EHF2[i+1] * 44100))
            section = np.zeros((round(EHF2[i+1] * 44100) - round(EHF2[i] * 44100)), dtype=np.int16)
            idx = 0
            for num in wave_data[round(EHF2[i] * 44100) : round(EHF2[i+1] * 44100)] :
                section[idx] = num
                idx += 1
                wavfile.writeframes(num)
        wav_sections.append(section)
        j = j + 1
        i = i + 2 
    print("section of the speech: ", len(wav_sections))
    return wav_sections, framerate


if __name__ == '__main__':
    print("Hi")
    cut_speech("./test/test-walk.wav")